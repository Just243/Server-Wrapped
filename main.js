import { STATS_CONFIG } from './assets/stats_config.js';
import { PLAYER_NAMES } from './assets/player_names.js';
import { AGGREGATE_CATEGORIES } from './assets/aggregate_categories.js';
...
let showPercentage = false; // Toggle for percentage display
        ...
        function showPlayerTopStats() {
            ...
            const processStats = (obj, prefix = '') => {
                for (const [key, value] of Object.entries(obj)) {
                    ...
                    if (typeof value === 'object' && !Array.isArray(value)) {
                        ...
                        flatStats.push({ name: fullKey, value });
                    }
                }
            };
            ...
            AGGREGATE_CATEGORIES.forEach(category => {
                ...
                if (aggValue > 0) {
                    flatStats.push({ name: category + ' (total)', value: aggValue });
                }
            });
                    ...
                    for (const [key, value] of Object.entries(obj)) {
                        ...
                        if (typeof value === 'object' && !Array.isArray(value)) {
                            ...
                            if (!allStatPaths.has(fullKey)) {
                                allStatPaths.set(fullKey, { isCategory: true, values: [] });
                            }
                            processStats(value, fullKey + '.');
                        } else if (typeof value === 'number') {
                            if (!allStatPaths.has(fullKey)) {
                                allStatPaths.set(fullKey, { isCategory: false, values: [] });
                            }
                            allStatPaths.get(fullKey).values.push({ player: player.name, value });
                        }
                    }
                };
        ...
        function viewAggregateLeaderboard(categoryPath) {
            ...
            allStats.forEach(player => {
                ...
                if (value > 0) {
                    aggregates.push({ player: player.name, value });
                }
            });
        ...
        function viewFullStat(statPath) {
            ...
            allStats.forEach(player => {
                ...
                if (value > 0) {
                    values.push({ player: player.name, value });
                }
            });
